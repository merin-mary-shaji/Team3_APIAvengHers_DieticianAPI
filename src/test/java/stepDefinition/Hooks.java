...
public class Hooks extends BaseUtils {
	...
	// private static final String SCENARIO_TagNAME_TO_LOG =
	...
	/* trying to skip the scenario which has output reponse as pdf file so that the log file looks cleaner
	 * @Before() public static void beforeScenario(Scenario scenario) throws
	 * FileNotFoundException {
	 * 
	 * 
	 * if (!logInitialized) { if (skipLogging(scenario)) {
	 * //scenario.log("Scenario is skipped based on the tag: " +
	 * SCENARIO_TagNAME_TO_LOG);
	 * 
	 * } else { PrintStream stream = new PrintStream(new
	 * FileOutputStream("Team3_APIAvengHers_log141.txt"));
	 * RestAssured.filters(RequestLoggingFilter.logRequestTo(stream));
	 * RestAssured.filters(ResponseLoggingFilter.logResponseTo(stream)); }
	 * logInitialized = true; }
	 * 
